...
import pandas as pd
import numpy as np
import CLIME_settings, CLIME_Process_Dataset, CLIME_perturbations
import re

    
def LIME_CLEAR(neighbour_seed):

    (X_test_sample,explainer,sensitivity_df,feature_list,numeric_features, model)=CLIME_Process_Dataset.Preproccess_Dataset(neighbour_seed,'lime comparison')
    ...
    #        qaz=sorted(exp.local_exp[1],key=lambda x: np.abs(x[0]))
        if CLIME_settings.case_study ==  'Credit Card':
            LIME_datarow, LIME_features = CLIME_Process_Dataset.Credit_categorical(data_row)    
            LIME_datarow= LIME_datarow.flatten()        
            lime_out= explainer.explain_instance(LIME_datarow, model, num_features=CLIME_settings.max_predictors,\
                                                 num_samples=15000, top_labels=None)
        elif CLIME_settings.case_study ==  'Census':   
            LIME_datarow, LIME_features = CLIME_Process_Dataset.Adult_categorical(data_row)    
            LIME_datarow= LIME_datarow.flatten()        
            lime_out= explainer.explain_instance(LIME_datarow, model, num_features=CLIME_settings.max_predictors,\
                                                 num_samples=15000, top_labels=None)

        else:
            lime_out= explainer.explain_instance(X_test_sample.iloc[i], model, num_features=CLIME_settings.max_predictors, top_labels=None)
    ...
    """ Counterfactual perturbations are now calculated and stored
        in the nncomp_df dataframe. If CLIME calculates a perturbation
        that is infeasible, then the details of the perturbation
        are stoted in the missing_log_df dataframe. CLIME will classify
