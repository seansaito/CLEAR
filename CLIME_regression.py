"""
Functions for CLIME to create local regressions
"""
...
import CLIME_settings,CLIME_perturbations
...
#creates synthetic data
        if CLIME_settings.case_study in ('PIMA Indians Diabetes','BreastC'):           
            self.master_df = pd.DataFrame(np.zeros(shape=(CLIME_settings.num_samples,\
                                        self.num_features)), columns=numeric_features)
            for i in numeric_features:
                self.master_df.loc[:,i] = np.random.uniform(self.feature_min[i], \
                                   self.feature_max[i], CLIME_settings.num_samples)  
# the synthetic data is now 'labelled' by the MLP model    
            LIME_pred_input_func = tf.estimator.inputs.pandas_input_fn(
                  ...
                  num_epochs=1,
                  shuffle=False)
            predictions = model.predict(LIME_pred_input_func)
        ...
        elif CLIME_settings.case_study in ('Census','Credit Card'):
            self.master_df = pd.DataFrame(np.zeros(shape=(CLIME_settings.num_samples, \
            ...
            for prefix in category_prefix:
                ...
                if len(cat_cols) >1:
                    ...
                    for cat in cat_cols:
                        if cnt == 0:
                            self.master_df[cat]=np.random.uniform(0, 1,CLIME_settings.num_samples)  
                            ...
                            v=CLIME_settings.num_samples-self.master_df[self.master_df[cat_cols].sum(axis=1)>99].shape[0]
...
#        if CLIME_settings.neighbourhood_algorithm != 'experiment':
#            save_master_df=True 
#            if save_master_df==True:
#                self.master_df.to_csv(CLIME_settings.LIME_path+'master_df.csv')
    ...
    def explain_data_point(self, data_row, observation_num):    
        ...
        if CLIME_settings.no_centering == True:
        ...
        LimeExplainer.create_neighbourhood(self)
        if CLIME_settings.apply_counterfactual_weights:
            LimeExplainer.add_counterfactual_rows(self)
            if self.num_counterf > 0:
                LimeExplainer.adjust_neighbourhood(self,self.neighbour_df.tail(self.num_counterf),\
                                                   CLIME_settings.counterfactual_weight) 
        LimeExplainer.perform_regression(self)
        if CLIME_settings.regression_type=='logistic':
            while  (self.additional_weighting < 6 and \
                   ((self.regression_class != self.nn_class) or \
                   (abs(self.local_prob-self.nn_forecast)> 0.01))):                 
                  LimeExplainer.adjust_neighbourhood(self,self.neighbour_df.iloc[0,:],10) 
                  LimeExplainer.perform_regression(self)
        return self   
    ...
    def adjust_neighbourhood(self, target_rows, num_copies):
        ...
        if CLIME_settings.generate_regression_files == True:
            filename1 = CLIME_settings.LIME_path +'adjlocal_'+datetime.now().strftime("%Y%m%d-%H%M%S%f")+'.csv'
...
# NEED TO REMOVE EXPERIMENT        
        if CLIME_settings.neighbourhood_algorithm == 'L2':
            ...
            num_rows = CLIME_settings.regression_sample_size *.4
            ...
            if CLIME_settings.generate_regression_files == True:
                filename1 = CLIME_settings.LIME_path +'local_'+str(self.observation_num) +'_' + datetime.now().strftime("%Y%m%d-%H%M%S%f")+'.csv'
                self.neighbour_df.to_csv(filename1)       
        
        elif CLIME_settings.neighbourhood_algorithm == 'L3':    
            ...
            self.local_df= self.local_df.sort_values(by=['distances'])
            self.local_df = self.local_df.reset_index(drop=True)
            num_rows = CLIME_settings.regression_sample_size *(neighbour_pt2/(neighbour_pt4-neighbour_pt1))
            temp_df=self.local_df[self.local_df['target_range']==1]
            temp_df= temp_df.sort_values(by=['distances'])
            temp_df = temp_df.iloc[0:int(num_rows),:]
            self.neighbour_df=temp_df.copy(deep= True)
            num_rows = int(CLIME_settings.regression_sample_size *(neighbour_pt3-neighbour_pt2)/(neighbour_pt4-neighbour_pt1))
            temp_df=self.local_df[self.local_df['target_range']==2]
            temp_df= temp_df.sort_values(by=['distances'])
            temp_df = temp_df.iloc[0:int(num_rows),:]
            self.neighbour_df=self.neighbour_df.append(temp_df,ignore_index=True)
            num_rows = int(CLIME_settings.regression_sample_size *(neighbour_pt4-neighbour_pt3)/(neighbour_pt4-neighbour_pt1))
            temp_df=self.local_df[self.local_df['target_range']==3]
            temp_df= temp_df.sort_values(by=['distances'])
            temp_df = temp_df.iloc[0:int(num_rows),:]
            self.neighbour_df=self.neighbour_df.append(temp_df,ignore_index=True)  
...
#sufficiently represented        
        elif CLIME_settings.neighbourhood_algorithm == 'Unbalanced':
            distances = sklearn.metrics.pairwise_distances(
                    self.local_df.iloc[:,0:self.num_features].values,
                    self.local_df.iloc[0,0:self.num_features].values.reshape(1,-1),
                    metric='euclidean'
        ...
        elif CLIME_settings.neighbourhood_algorithm== 'L1':
            ...
            if CLIME_settings.generate_regression_files == True:
                filename1 = CLIME_settings.LIME_path +'local_'+datetime.now().strftime("%Y%m%d-%H%M%S%f")+'.csv'
    ...
    def add_counterfactual_rows(self):
        ...
        for feature in self.neighbour_df.columns[0:-3]:   
              ...
              if (c_df['min']<=0.5) & (c_df['max']>0.5):           
                ...
                perc50=CLIME_perturbations.Get_Counterfactual(self.sensitivity_df,feature, old_value, self.observation_num)
...
#transform neighbourhood data so that it passes through the data point to be explained
        ...
        X = X.reset_index(drop=True)
        if CLIME_settings.regression_type in ['logistic','multiple']:
            decision_threshold = 0.5
            if CLIME_settings.with_indicator_feature == True:
                indicatorFeature_value = np.where((X[CLIME_settings.feature_with_indicator]>=CLIME_settings.indicator_threshold), 1, 0)
            ...
            if CLIME_settings.no_polynomimals ==True:
                poly_df = X.copy(deep=True)
            else:
                if CLIME_settings.interactions_only ==True:
...
#NOw transform so that regression goes through the data point to be explained            
            if CLIME_settings.regression_type=='multiple' and CLIME_settings.no_centering==False:
...
#stepwise regression's choice of variables is restricted, but this was found to improve fidelity.            
            if CLIME_settings.case_study =='PIMA Indians Diabetes':
                ...
                try:
                    ...
                    pass
            if CLIME_settings.case_study =='BreastC':
                #selected =['1', 'Pregnancies', 'Glucose', 'BloodPressure', 'SkinThickness', 'Insulin', 'BMI', 'DiabetesPedigreeFunction', 'Age']
...
#                           'concavepoints','symmetry','fractaldimension']
                ...
                try:    
                    ...
                    pass
            elif CLIME_settings.case_study =='Census':
                ...
                remaining = poly_df.columns.tolist()         
            elif CLIME_settings.case_study =='Credit Card':
...
#LIME                selected= ['1','PAY0', 'PAYAMT1', 'PAYAMT2', 'marDd2', 'BILLAMT1', 'PAYAMT5', 'LIMITBAL', 'PAYAMT3', 'BILLAMT6', 'PAY2', 'PAY4', 'BILLAMT2', 'PAY5', 'genDd1', 'BILLAMT3', 'BILLAMT4', 'PAYAMT4', 'PAYAMT6']
            ...
            while remaining and current_score == best_new_score and len(selected)<CLIME_settings.max_predictors:
                scores_with_candidates = []
                for candidate in remaining:
                    if CLIME_settings.regression_type=='multiple' and CLIME_settings.no_centering == False:
                        formula = "{} ~ {}".format('prediction',' + '.join(selected+ [candidate])+'-1')
                    elif CLIME_settings.regression_type=='multiple' and CLIME_settings.no_centering == True:
...
#                        file.write(formula + '\n')
                    try:
                        if CLIME_settings.score_type == 'aic':
                            if CLIME_settings.regression_type=='multiple':
                                score = sm.GLS.from_formula(formula, poly_df).fit(disp=0).aic
                            else:
                                score = sm.Logit.from_formula(formula, poly_df).fit(disp=0).aic 
                            score = score *-1
                        elif CLIME_settings.score_type == 'prsquared':
                            if CLIME_settings.regression_type=='multiple':
                        ...
                        elif CLIME_settings.score_type == 'adjR':
                            if CLIME_settings.regression_type=='multiple':
            ...
            try:
                ...
                if CLIME_settings.regression_type=='logistic' or \
                   (CLIME_settings.regression_type=='multiple' and CLIME_settings.no_centering == True):
...
def Run_Regressions(X_test_sample,explainer,feature_list):    
    """ Creates an object of class LimeExplainer. This creates the Synthetic Dataset
        and calculates some statistics for the variables in X_train
    """    
   
    """  Creates an object of class LimeExplainer.explain_data_point which performs
    ...
    observation_num = CLIME_settings.first_obs       
    for i in range(CLIME_settings.first_obs,CLIME_settings.last_obs):
        ...
        observation_num += 1  
    filename1 = CLIME_settings.LIME_path +'LIME_'+ datetime.now().strftime("%Y%m%d-%H%M")+'.csv'   
#    filename2 = CLIME_settings.LIME_path +'Results_'+ datetime.now().strftime("%Y%m%d-%H%M")+'.pkl'  
